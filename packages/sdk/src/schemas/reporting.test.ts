...
describe('reporting schemas', () => {
  ...
  it('accepts keyed key-number responses', () => {
    ...
    const keyed = parsed as Record<string, { nb_visits: number }>;
  ...
  it('retains unknown properties via passthrough', () => {
    ...
    expect((urlRow as { custom?: string }).custom).toBe('value');
    ...
    expect((refRow as { extra?: number }).extra).toBe(123);
    ...
    expect((eventRow as { nested?: { some: string } }).nested?.some).toBe('value');
  });
});
